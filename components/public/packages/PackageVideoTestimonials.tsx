...
interface PackageVideoTestimonialsProps {
  testimonials: TestimonialItem[];
}

export default function PackageVideoTestimonials({ testimonials }: PackageVideoTestimonialsProps) {
  ...
  return (
    <section className="bg-white py-16 rounded-2xl border border-gray-border shadow-card overflow-hidden">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="relative w-full max-w-4xl mx-auto flex items-center justify-center">
          ...
          {testimonials.length > 1 && (
            <button
              onClick={prevSlide}
              className="absolute left-[-20px] md:left-[-60px] z-30 w-11 h-11 rounded-full bg-primary hover:bg-primary-dark text-white flex items-center justify-center shadow-orange cursor-pointer hover:scale-105 transition-all border-none"
            >
          ...
          <div className="w-full bg-white border border-gray-border rounded-3xl overflow-hidden shadow-card hover:shadow-float relative aspect-[16/11] md:aspect-[16/10]">
            <AnimatePresence initial={false} custom={direction}>
              {testimonials.map((t, idx) => {
                ...
                const thumbUrl = t.thumbnail || (ytId ? `https://img.youtube.com/vi/${ytId}/maxresdefault.jpg` : "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800&q=80");
          ...
          {testimonials.length > 1 && (
            <button
              onClick={nextSlide}
              className="absolute right-[-20px] md:right-[-60px] z-30 w-11 h-11 rounded-full bg-primary hover:bg-primary-dark text-white flex items-center justify-center shadow-orange cursor-pointer hover:scale-105 transition-all border-none"
            >
